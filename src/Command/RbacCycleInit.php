...
final class RbacCycleInit extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        /** @var Table $table */
        if ($reCreate && $this->dbal
                ->database()
                ->hasTable($itemsChildrenTable) === true) {
            $this->dropTable($itemsChildrenTable);
        }
        if ($reCreate && $this->dbal
                ->database()
                ->hasTable($this->config['assignmentsTable']) === true) {
            $this->dropTable($this->config['assignmentsTable']);
        }
        if ($reCreate && $this->dbal
                ->database()
                ->hasTable($this->config['itemsTable']) === true) {
            $this->dropTable($this->config['itemsTable']);
        }
        if ($this->dbal
                ->database()
                ->hasTable($this->config['itemsTable']) === false) {
        ...
        if ($this->dbal
                ->database()
                ->hasTable($itemsChildrenTable) === false) {
        ...
        if ($this->dbal
                ->database()
                ->hasTable($this->config['assignmentsTable']) === false) {
